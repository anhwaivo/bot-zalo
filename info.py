        ...
        if message.startswith(".info"):
            ...
            try:
                ...
                if message_object.mentions:
                    args = message_object.mentions[0]['uid']
                elif message[5:].strip().isnumeric():
                    args = message[5:].strip()
                    if (len(args) == 10):
                        sdt=True
                elif message.strip() == ".info":
                    args = author_id
                else:
                    self.send(Message(text="invalid args", style=style_error), thread_id, thread_type)
                    return
                
                if (sdt):
                    msg = "Bussiness va bio con sida, ae thong cam \n"
                    multistyle = []
                    try:
                        info = self.fetchPhoneNumber(args)
                        #info = info.unchanged_profiles or info.changed_profiles

                        if info is self._undefined:
                            self.send(Message(text="err sdt 1", style=style_error), thread_id, thread_type)
                            return

                        class User:
                            def __init__(this, avatar, cover, status, gender, dob, sdob, globalId, bizPkg, uid, zalo_name, display_name):
                                this.avatar = avatar
                                this.cover = cover
                                this.status = status
                                this.gender = gender
                        ...
                        userName = info.zalo_name[:30] + "..." if len(info.zalo_name) > 30 else info.zalo_name
                        ...
                        self.send(Message(text="err userid 2", style=style_error), thread_id, thread_type)
                else:
                    msg = "bio con sida, ae thong cam \n"
                    multistyle = []
                    try:
                        info = self.fetchUserInfo(args)
                        info = info.unchanged_profiles or info.changed_profiles
                        if info is self._undefined:
                ...
                print(f"Error: {e}")
                self.send(Message(text="err nguyen 1 dong code", style=style_error), thread_id, thread_type)
